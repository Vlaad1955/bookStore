...
import {CreateCategoryDto, FetchCategoriesOptions, UpdateCategoryDto} from "@/shared/types/categoryTypes/category";
...
export async function getCategoryList(queryParams:FetchCategoriesOptions) {
  const response = await axiosInstance.get("/category/list", { params: queryParams });
  return response.data;
}
